...
async function getTemplate(moduleId,templateId) {
    console.log('data',moduleId)
    
    if(moduleID && templateId){
        let response = await fetch(`https://api.wolfeocrm.com/v1/setting/${moduleId.typeId}/form-template/${templateId.formId}`, {
            method: 'GET',
            headers: {
